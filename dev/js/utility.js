...
Utility.calcLatLonDistances = function (Latitude_In_Degrees) {
  ...
  var lat = Utility.deg2rad(Latitude_In_Degrees); // Set up "Constants"
  ...
  var longlen = p1 * Math.cos(lat) + p2 * Math.cos(3 * lat) + p3 * Math.cos(5 * lat); // Distances in meters
...
Utility.metersToFeet = function (meters) {
  ...
  // convert to feet
  var feet = meters * 3.28084; // if rounded is true then round to nearest foot
...
Utility.metersToMiles = function (meters) {
  var rounded = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
  // convert to feet
  var miles = meters * 0.0006214; // if rounded is true then round to nearest mile
...
Utility.curveDistance = function (distance) {// Takes a straight-line distance and returns the actual distance when converted into  a distance between two points on a circle. See https://en.wikipedia.org/wiki/Great-circle_distance
...
Utility.calculateETA = function (distance, speed) {// This distance is not linear. It is an arch because of the earths curvature. See https://en.wikipedia.org/wiki/Great-circle_distance
